...
消息模块已支持会话列表、消息明细和文本发送，并接入 `Socket.IO` 做演示级实时追加；消息发送同时会写入 `message.lifecycle` 事件，为后续未读数、在线状态和推送通知预留统一入口。校园服务子模块围绕跑腿、代办、拼单和临时帮忙展开，已具备发布、接单、完成和消息联动能力。后台侧则覆盖商品审核、举报处理、用户封禁与解封等治理动作，同时把审计留痕收口到 `governance.audit` 事件，由 `backend` 发布到 RabbitMQ，再由独立 `governance-worker` 消费。商品浏览、联系、收藏等用户行为也已开始通过 `recommendation.behavior` 事件沉淀到推荐画像。商品与校园服务在发布前会先经过本地规则校验，再按配置决定是否进入 `DeepSeek` 二次审核。
...
架构层面保留了前后端分离、单体后端按领域模块拆分、Prisma 统一数据访问、文档与代码同仓维护以及事件驱动能力预留等约束。除搜索和电商同步外，消息和推荐先在单体内收口为 Outbox 事件和本地 consumer；治理则已经形成 `Outbox + RabbitMQ + governance-worker` 的独立异步链路，为后续继续拆服务准备契约和处理边界。
...
- 默认运行态服务是 `mysql`、`minio`、`meilisearch`、`rabbitmq`、`supertokens-db`、`supertokens`、`vendure`、`backend`、`search-indexer`、`commerce-sync`、`governance-worker`、`frontend`
...
- 消息和推荐仍在 `backend` 进程内消费各自的 Outbox 主题，已经完成事件化；治理审计已切到 `backend -> RabbitMQ -> governance-worker`
...
- 治理 MQ 配置：`GOVERNANCE_MQ_ENABLED`、`RABBITMQ_URL`、`GOVERNANCE_MQ_EXCHANGE`、`GOVERNANCE_MQ_QUEUE`、`GOVERNANCE_MQ_ROUTING_KEY`、`GOVERNANCE_MQ_PREFETCH`
