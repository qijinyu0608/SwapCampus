...
import { clearDemoUser, hasAdminAccess, isGuestUser, type DemoUser } from '../../services/session';
...
function IconLink({
  ...
  return (
    <Tooltip title={label}>
      <Link to={to} className={active ? 'topbar-icon-button active' : 'topbar-icon-button'} aria-label={label}>
        {icon}
      </Link>
    </Tooltip>
  );
...
export function TopBar({ currentUser }: TopBarProps) {
  ...
  const leftItems: NavIconItem[] = [
    ...
    },
    {
      ...
      label: '想要',
      icon: <StarOutlined />,
      activeMatch: (pathname) => pathname.startsWith('/favorites')
    },
    {
      key: 'messages',
      to: '/messages',
  ...
  const rightItems: NavIconItem[] = adminMode
    ...
    : [
        {
          key: 'publish',
          to: guestMode ? '/login' : '/publish',
          label: guestMode ? '去登录' : '发布商品',
        ...
        },
        {
          ...
          label: currentUser?.name ?? '登录',
          icon: currentUser ? <UserOutlined /> : <LoginOutlined />,
          activeMatch: (pathname) => pathname.startsWith('/profile') || pathname.startsWith('/login')
        }
      ];
  ...
  function updateLogoHighlight(event: MouseEvent<HTMLAnchorElement>) {
    const bounds = event.currentTarget.getBoundingClientRect();
    const x = ((event.clientX - bounds.left) / bounds.width) * 100;
    const y = ((event.clientY - bounds.top) / bounds.height) * 100;
    event.currentTarget.style.setProperty('--logo-glow-x', `${x.toFixed(2)}%`);
    event.currentTarget.style.setProperty('--logo-glow-y', `${y.toFixed(2)}%`);
  }

  function resetLogoHighlight(event: MouseEvent<HTMLAnchorElement>) {
    event.currentTarget.style.setProperty('--logo-glow-x', '50%');
    event.currentTarget.style.setProperty('--logo-glow-y', '50%');
  }
    ...
    <div className="topbar-shell">
      ...
      <div className="topbar-center">
        <Link
          ...
          aria-label="SwapCampus 首页"
          onMouseMove={updateLogoHighlight}
          onMouseLeave={resetLogoHighlight}
        >
          <svg className="topbar-logo-svg" viewBox={LOGO_VIEWBOX} aria-hidden="true" focusable="false">
            <defs>
              <radialGradient id="topbar-logo-highlight" cx="50%" cy="50%" r="54%">
                <stop offset="0%" stopColor="var(--signature-highlight-core)" />
                <stop offset="34%" stopColor="var(--signature-highlight-edge)" />
                <stop offset="62%" stopColor="var(--signature-highlight-halo)" />
      ...
      <div className="topbar-cluster topbar-cluster-right">
        ...
        {currentUser ? (
          <Tooltip title="退出登录">
            <button type="button" className="topbar-icon-button" aria-label="退出登录" onClick={handleLogout}>
              <LogoutOutlined />
            </button>
          </Tooltip>
        ) : null}
